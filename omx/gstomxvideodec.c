  ...
  if (peer) {
    ...
    for (n = 0; n < cnt; n++) {
      ...
      if (gst_caps_features_contains (feature, "memory:GLMemory")) {
        ...
        GST_DEBUG_OBJECT (self, "Setting decoder to send out GLMemory");
        return BUF_EGL;
...
#if defined (USE_OMX_TARGET_TEGRA)
  {
    ...
    if (nv_buf == BUF_EGL) {
      state = gst_video_decoder_set_output_state (GST_VIDEO_DECODER (self),
          ...
          port_def.format.video.nFrameHeight, self->input_state);
      state->caps = gst_video_info_to_caps (&state->info);
      features = gst_caps_features_new ("memory:GLMemory", NULL);
      gst_caps_set_features (state->caps, 0, features);

      if (gst_video_decoder_negotiate (GST_VIDEO_DECODER (self))) {
        self->eglimage = TRUE;
      } else {
        GST_ERROR_OBJECT (self, "Failed to negotiate RGBA for GLMemory");
        self->eglimage = FALSE;
      }
    }
#else
    ...
    if (caps && gst_video_info_from_caps (&info, caps)
      ...
      for (i = 0; i < n; i++) {
        ...
        if (allocator) {
          ...
          if (allocator_found) {
            GST_DEBUG_OBJECT (self, "Found compatibile GLMemory allocator");
            break;
          }
        }
      }
      ...
      for (i = 0; i < n; i++) {
...

        {
          ...
          if (!gst_buffer_pool_config_get_allocator (config, &allocator, NULL)) {
            GST_TRACE_OBJECT (self, "pool config has no allocator");
            goto next_pool;
          }
          ...
          if (!GST_IS_GL_MEMORY_EGL_ALLOCATOR (allocator)) {
            ...
            if (!allocator) {
              ...
              GST_TRACE_OBJECT (self, "No GLMemoryEGL allocator");
              break;
            }

            gst_buffer_pool_config_set_allocator (config, allocator, NULL);
            if (!gst_buffer_pool_set_config (pool, config)) {
              GST_TRACE_OBJECT (self, "pool failed configuration with new "
                  "allocator");
              goto next_pool;
            }
            allocator_found = TRUE;
        ...
        GST_DEBUG_OBJECT (self, "Found compatibile GLBufferPool");
