...
static gboolean gst_omx_amrnb_dec_set_format (GstOMXAudioDec * decoder,
...
G_DEFINE_TYPE_WITH_CODE (GstOMXAMRNBDec, gst_omx_amrnb_dec,
    ...
    GST_DEBUG_CATEGORY_INIT (gst_omx_amrnb_dec_debug_category, "omxamrnbdec", 0,
        "debug category for omxamrnbdec element"));
...
gst_omx_amrnb_dec_class_init (GstOMXAMRNBDecClass * klass)
{
  ...
  omxdec_class->set_format = GST_DEBUG_FUNCPTR (gst_omx_amrnb_dec_set_format);

  omxdec_class->cdata.default_sink_template_caps = "audio/AMR";

  gst_element_class_set_static_metadata (GST_ELEMENT_CLASS (klass),
      "OpenMax AMR-NB decoder", "Codec/Decoder/Audio",
      "Decodes AMR-NB audio streams", "Jitendra Kumar <jitendrak@nvidia.com>");

  gst_omx_set_default_role (&omxdec_class->cdata, "audio_decoder.amrnb");
}

static void
gst_omx_amrnb_dec_init (GstOMXAMRNBDec * omxamrnbdec)
{
}

static gboolean
gst_omx_amrnb_dec_set_format (GstOMXAudioDec * decoder, GstOMXPort * port,
    GstCaps * caps)
{
  GstOMXAMRNBDec *self = GST_OMX_AMRNB_DEC (decoder);
